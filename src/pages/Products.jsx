  ...
  return (
    <div className="animate-in">
      ...
      <section className="container" style={{ padding: '0 20px 48px' }}>
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '64px 24px', border: '1px dashed var(--border-medium)', borderRadius: '18px', color: 'var(--text-muted)' }}>
            {isAdminMode
              ? 'Aucune prestation. Créez-en une depuis le dashboard → « Prestations ».'
              : 'Nos prestations arrivent bientôt.'}
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '20px' }}>
            {products.map((p, i) => (
              <FadeIn key={p.id} direction="up" delay={Math.min(i * 0.06, 0.4)}>
                <Link
                  to={`/prestations/${p.slug}`}
                  aria-label={`Découvrir : ${p.name}`}
                  style={{ textDecoration: 'none', display: 'block', height: '100%' }}
                >
                  <article
                    className="album-card"
                    style={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: '18px', overflow: 'hidden', border: '1px solid var(--border-light)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-md)', opacity: p.visible === false ? 0.5 : 1 }}
                  >
                    <div style={{ position: 'relative', aspectRatio: '4 / 3', overflow: 'hidden', background: 'var(--bg-secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      ...
                      {p.badge && (
                        <span style={{ position: 'absolute', top: '10px', left: '10px', background: 'var(--primary)', color: '#fff', fontSize: '11px', fontWeight: 800, padding: '4px 10px', borderRadius: '999px' }}>{p.badge}</span>
                      )}
                    </div>
                    <div style={{ padding: '16px', flexGrow: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      <h2 style={{ fontSize: '18px', fontWeight: 800, color: 'var(--text-main)', margin: 0, lineHeight: 1.3 }}>{p.name}</h2>
                      {p.tagline && <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: 0, lineHeight: 1.5, flexGrow: 1 }}>{p.tagline}</p>}
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '8px' }}>
                        <span style={{ fontSize: '15px', fontWeight: 800, color: 'var(--primary)' }}>
                          {p.priceFrom != null ? `Dès ${p.priceFrom}€` : 'Sur devis'}
                        </span>
